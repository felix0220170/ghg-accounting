...
const INDICATORS = [
  ...
  },
  {
    ...
    decimalPlaces: 2,
    getValue: (data) => {
      const { cokingAmount, carbonContent, carbonOxidationRate } = data;
      return cokingAmount * carbonContent / 100 * (carbonOxidationRate / 100) * 44 / 12;
    }
  }
];
...
function PetroleumCokingPlantEmission({ onEmissionChange }) {
  // 催化裂化装置列表状态
    ...
    setCrackingUnits(prevUnits => {
      ...
      const updatedUnits = prevUnits.map(unit => {
        ...
        for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
          ...
          // 获取当月的烧焦量
          const cokingAmountData = unit.data.cokingAmount?.find(m => m.month === month);
          const cokingAmountValue = cokingAmountData?.value ? parseFloat(cokingAmountData.value) : 0;
          ...
          // 计算排放量：烧焦量 * 焦层中含碳量 / 100 * (碳氧化率/100) * 44 / 12
          const emissionValue = cokingAmountValue * carbonContentValue / 100 * (carbonOxidationRateValue / 100) * 44 / 12;
      ...
      <div style={{ marginTop: '20px',marginBottom: '40px' }}>
        <div style={{ marginBottom: '20px' }}>
          <div style={{ backgroundColor: '#f5f5f5', padding: '16px', borderRadius: '4px', lineHeight: '1.2' }}>
            ...
            <p>炼油厂使用的焦化装置可以分为延迟焦化装置、流化焦化装置和灵活焦化装置三种形式</p>
            <p>延迟焦化装置和灵活焦化装置都按都归在燃料燃烧排放中， 流化焦化装置中流化床燃烧器烧除附着在焦炭粒子上的多余焦
炭所产生的CO2 排放 参照 催化裂化装置连续烧焦排放的计算方法进行核算</p>
            <h3 style={{ marginBottom: '12px' }}>计算说明</h3>
            <p>排放量（tCO₂） = 烧焦量（t） × 焦层中含碳量（%） × (碳氧化率（%）) × 44 / 12</p>
            <p>- 烧焦量单位为 t，保留两位小数</p>
        ...
        <div style={{ marginTop: '24px', backgroundColor: '#fff', padding: '20px', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)', marginBottom: '24px' }}>
          <h3 style={{ marginBottom: '20px', color: '#1890ff', fontWeight: 'bold', fontSize: '18px' }}>添加焦化装置烧焦排放记录</h3>
            ...
            <div key={unit.id} style={{ marginBottom: '32px', border: '1px solid #e8e8e8', padding: '16px', borderRadius: '4px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  <h3 style={{ margin: 0, fontSize: '16px', marginRight: '16px' }}>焦化装置 {index + 1}</h3>
                </div>
                {(
                  <button
...
export default PetroleumCokingPlantEmission;
