...
const INDICATORS = [
  ...
  },
  {
    ...
    decimalPlaces: 2,
    getValue: (data) => {
      const { asphaltProduction, emissionFactor } = data;
      return asphaltProduction * emissionFactor;
    }
  }
];
...
// 为氧化沥青装置初始化数据（纵向布局）
const initializeAsphaltUnitData = (unitNumber) => {
  ...
  return {
    id: `asphalt-unit-${Date.now()}`,
    unitNumber: unitNumber,
    name: `氧化沥青装置 ${unitNumber}`,
    data: initialData,
    files: {}
  };
};

function PetroleumOxidizedAsphaltEmission({ onEmissionChange }) {
  // 氧化沥青装置列表状态
  const [asphaltUnits, setAsphaltUnits] = useState([]);
  
  // 保存上一次的总排放量，用于比较是否真正发生变化
  const previousEmissionRef = useRef(null);
  
  // 添加新的氧化沥青装置
  const addNewAsphaltUnit = useCallback(() => {
    const newUnitNumber = asphaltUnits.length + 1;
    const newAsphaltUnit = initializeAsphaltUnitData(newUnitNumber);
    setAsphaltUnits(prevUnits => [...prevUnits, newAsphaltUnit]);
  }, [asphaltUnits]);
  
  // 移除氧化沥青装置
  const removeAsphaltUnit = useCallback((unitId) => {
    setAsphaltUnits(prevUnits => prevUnits.filter(unit => unit.id !== unitId));
  }, []);
    ...
    setAsphaltUnits(prevUnits => {
      ...
      const updatedUnits = prevUnits.map(unit => {
        ...
        for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
          ...
          // 获取当月的氧化沥青产量
          const asphaltProductionData = unit.data.asphaltProduction?.find(m => m.month === month);
          const asphaltProductionValue = asphaltProductionData?.value ? parseFloat(asphaltProductionData.value) : 0;
          ...
          // 计算排放量：氧化沥青产量 * 排放系数
          const emissionValue = asphaltProductionValue * emissionFactorValue;
  ...
  }, [updateCalculatedValues, asphaltUnits]);
  
  // 处理数据变化（纵向布局）
  const handleDataChange = useCallback((id, indicatorKey, month, field, value, type = 'asphalt-unit') => {
    // 不进行格式化，直接使用原始输入值
    const formattedValue = value || '';
    
    // 使用氧化沥青装置相关设置
    const setState = setAsphaltUnits;
    const indicators = INDICATORS;
  ...
  // 处理文件上传
  const handleFileUpload = useCallback((id, indicatorKey, month, file, type = 'asphalt-unit') => {
    if (!file) return;
    ...
    const tableHeaders = (
      <thead>
        <tr style={{ backgroundColor: '#f5f5f5' }}>
          <th style={{ border: '1px solid #d9d9d9', padding: '8px', textAlign: 'center', minWidth: '120px'}}>指标名称</th>
  ...
  const renderTotalEmissionTable = () => {
    ...
    return (
      <table style={{ borderCollapse: 'collapse', width: '100%', marginTop: '24px', backgroundColor: '#f9f9f9' }}>
        ...
        <tbody>
          <tr>
            <td style={{ border: '1px solid #d9d9d9', padding: '8px', fontWeight: 'bold' }}>氧化沥青装置排放总量</td>
  ...
  return (
    <div style={{ padding: '16px' }}>
      <h2 style={{ marginBottom: '24px', fontSize: '20px', color: '#333' }}>氧化沥青装置排放</h2>
      ...
      <div style={{ marginTop: '20px', marginBottom: '40px' }}>
        <div style={{ marginBottom: '20px' }}>
          <div style={{ backgroundColor: '#f5f5f5', padding: '16px', borderRadius: '4px', lineHeight: '1.2' }}>
            ...
            <p>氧化沥青可使沥青聚合和稳定化，增加沥青用于铺裹屋顶和墙面装修的耐气候性。氧化沥青工艺CH4 排放量很小，因为蒸馏过程已经脱除了大部分的轻质烃</p>
            <p>排放量计算公式：CO₂排放 = 氧化沥青产量 × 排放系数</p>
            <p>排放系数：单位氧化沥青产量所产生的二氧化碳排放量，默认值为0.05 tCO₂/t。</p>
          </div>
        </div>
        ...
        {/* 氧化沥青装置列表 */}
        {asphaltUnits.map((unit) => (
          <div key={unit.id} style={{ marginBottom: '32px', backgroundColor: '#fff', border: '1px solid #d9d9d9', borderRadius: '8px', padding: '16px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <h3 style={{ margin: 0, fontSize: '18px' }}>{unit.name}</h3>
              <button
                onClick={() => removeAsphaltUnit(unit.id)}
                style={{
...
export default PetroleumOxidizedAsphaltEmission;