...
const INDICATORS = [
  ...
  },
  {
    ...
    decimalPlaces: 2,
    getValue: (data) => {
      const { exhaustFlowRate, totalBurningTime, co2Concentration, coConcentration } = data;
      // CO2排放 = 炉管烧焦尾气平均流量 * 累计烧焦时间 * (CO2的体积浓度 + CO的体积浓度) * 19.7 * 0.0001
      return exhaustFlowRate * totalBurningTime * (co2Concentration + coConcentration) * 19.7 * 0.0001;
    }
  }
];
...
// 为乙烯裂解装置初始化数据（纵向布局）
const initializeEthyleneCrackingUnitData = (unitNumber) => {
  ...
  return {
    id: `ethylene-cracking-unit-${Date.now()}`,
    unitNumber: unitNumber,
    name: `乙烯裂解装置 ${unitNumber}`,
    data: initialData,
    files: {}
  };
};

function PetroleumEthyleneCrackingEmission({ onEmissionChange }) {
  // 乙烯裂解装置列表状态
  ...
  const addNewCrackingUnit = useCallback(() => {
    ...
    const newCrackingUnit = initializeEthyleneCrackingUnitData(newUnitNumber);
    ...
    setCrackingUnits(prevUnits => {
      ...
      const updatedUnits = prevUnits.map(unit => {
        ...
        for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
          ...
          // 获取当月的炉管烧焦尾气平均流量
          const exhaustFlowRateData = unit.data.exhaustFlowRate?.find(m => m.month === month);
          const exhaustFlowRateValue = exhaustFlowRateData?.value ? parseFloat(exhaustFlowRateData.value) : 0;
          
          // 获取当月的累计烧焦时间
          const totalBurningTimeData = unit.data.totalBurningTime?.find(m => m.month === month);
          const totalBurningTimeValue = totalBurningTimeData?.value ? parseFloat(totalBurningTimeData.value) : 0;

          // 获取当月的炉管烧焦尾气中CO₂的体积浓度
          const co2ConcentrationData = unit.data.co2Concentration?.find(m => m.month === month);
          const co2ConcentrationValue = co2ConcentrationData?.value ? parseFloat(co2ConcentrationData.value) : 0;
          
          // 获取当月的烧焦尾气中CO的体积浓度
          const coConcentrationData = unit.data.coConcentration?.find(m => m.month === month);
          const coConcentrationValue = coConcentrationData?.value ? parseFloat(coConcentrationData.value) : 0;
          
          // 计算排放量：CO2排放 = 炉管烧焦尾气平均流量 * 累计烧焦时间 * (CO2的体积浓度 + CO的体积浓度) * 19.7 * 0.0001
          const emissionValue = exhaustFlowRateValue * totalBurningTimeValue * (co2ConcentrationValue + coConcentrationValue) /100 * 19.7 * 0.0001;
  ...
  // 处理数据变化（纵向布局）
  const handleDataChange = useCallback((id, indicatorKey, month, field, value, type = 'ethylene-cracking-unit') => {
    // 不进行格式化，直接使用原始输入值
  ...
  // 处理文件上传
  const handleFileUpload = useCallback((id, indicatorKey, month, file, type = 'ethylene-cracking-unit') => {
    if (!file) return;
  ...
  const renderTotalEmissionTable = () => {
    ...
    return (
      <table style={{ borderCollapse: 'collapse', width: '100%', marginTop: '24px', backgroundColor: '#f9f9f9' }}>
        ...
        <tbody>
          <tr>
            <td style={{ border: '1px solid #d9d9d9', padding: '8px', fontWeight: 'bold' }}>乙烯裂解装置排放总量</td>
  ...
  return (
    <div style={{ padding: '16px' }}>
      <h2 style={{ marginBottom: '24px', fontSize: '20px', color: '#333' }}>乙烯裂解装置排放</h2>
      ...
      <div style={{ marginTop: '20px',marginBottom: '40px' }}>
        <div style={{ marginBottom: '20px' }}>
          <div style={{ backgroundColor: '#f5f5f5', padding: '16px', borderRadius: '4px', lineHeight: '1.2' }}>
            ...
            <p>乙烯裂解装置的工业生产过程排放来自于炉管内壁结焦后的烧焦排放，排放量可根据烧焦过程中炉管排气口的气体流量及其中的CO2及CO 浓度确定</p>
            <p>排放量计算公式：CO₂排放 = 炉管烧焦尾气平均流量 * 累计烧焦时间 * (CO₂的体积浓度 % + CO的体积浓度 %) * 19.7 * 0.0001</p>
          </div>
        </div>
        ...
        {crackingUnits.length > 0 ? (
          ...
          <p style={{ color: '#999' }}>暂无乙烯裂解装置数据，请点击上方按钮添加。</p>
        )}
      </div>

      {/* 总排放量统计 */}
      {crackingUnits.length > 0 && renderTotalEmissionTable()}
    </div>
  );
}

export default PetroleumEthyleneCrackingEmission;