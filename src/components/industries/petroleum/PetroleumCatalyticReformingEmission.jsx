...
const INDICATORS = [
  ...
  },
  {
    ...
    getValue: (data) => {
      ...
      return (catalystAmount * (1 - carbonContentBefore / 100) * (carbonContentBefore / (100 - carbonContentBefore) - carbonContentAfter / (100 - carbonContentAfter)) * 44 / 12);
    }
  }
];
...
// 为催化重整装置初始化数据（纵向布局）
const initializeReformingUnitData = (unitNumber) => {
  ...
  return {
    id: `reforming-unit-${Date.now()}`,
    unitNumber: unitNumber,
    name: `催化重整装置 ${unitNumber}`,
    data: initialData,
    files: {}
  };
};

function PetroleumCatalyticReformingEmission({ onEmissionChange }) {
  // 催化重整装置列表状态
  const [reformingUnits, setReformingUnits] = useState([]);
  
  // 保存上一次的总排放量，用于比较是否真正发生变化
  const previousEmissionRef = useRef(null);
  
  // 添加新的催化重整装置
  const addNewReformingUnit = useCallback(() => {
    const newUnitNumber = reformingUnits.length + 1;
    const newReformingUnit = initializeReformingUnitData(newUnitNumber);
    setReformingUnits(prevUnits => [...prevUnits, newReformingUnit]);
  }, [reformingUnits]);
  
  // 移除催化重整装置
  const removeReformingUnit = useCallback((unitId) => {
    setReformingUnits(prevUnits => prevUnits.filter(unit => unit.id !== unitId));
  }, []);
    ...
    setReformingUnits(prevUnits => {
      ...
      const updatedUnits = prevUnits.map(unit => {
        ...
        for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
          ...
          // 使用与INDICATORS中相同的计算公式
          // 排放量（tCO₂） = 待再生的催化剂量 × (1 - 再生前催化剂含碳量 / 100) × (再生前催化剂含碳量 / (100 - 再生前催化剂含碳量) - 再生后催化剂含碳量 / (100 - 再生后催化剂含碳量)) × 44 / 12
  ...
  }, [updateCalculatedValues, reformingUnits]);
  
  // 处理数据变化（纵向布局）
  const handleDataChange = useCallback((id, indicatorKey, month, field, value, type = 'reforming-unit') => {
    // 不进行格式化，直接使用原始输入值
    const formattedValue = value || '';
    
    // 使用催化重整装置相关设置
    const setState = setReformingUnits;
    const indicators = INDICATORS;
  ...
  // 处理文件上传
  const handleFileUpload = useCallback((id, indicatorKey, month, file, type = 'reforming-unit') => {
    if (!file) return;
  ...
  const renderTotalEmissionTable = () => {
    ...
    return (
      <table style={{ borderCollapse: 'collapse', width: '100%', marginTop: '24px', backgroundColor: '#f9f9f9' }}>
        ...
        <tbody>
          <tr>
            <td style={{ border: '1px solid #d9d9d9', padding: '8px', fontWeight: 'bold' }}>催化重整装置排放总量</td>
  ...
  return (
    <div style={{ padding: '16px' }}>
      <h2 style={{ marginBottom: '24px', fontSize: '20px', color: '#333' }}>催化重整装置催化剂烧焦排放</h2>
      ...
      <div style={{ marginTop: '20px',marginBottom: '40px' }}>
        <div style={{ marginBottom: '20px' }}>
          <div style={{ backgroundColor: '#f5f5f5', padding: '16px', borderRadius: '4px', lineHeight: '1.2' }}>
            ...
            <p>催化重整是指在一定的温度和压力及催化剂作用下，烃分子发生
重新排列，使环烷烃和烷烃转化成芳烃和异构烷烃，生产高辛烷值汽
油及轻芳烃(苯、甲苯、二甲苯)的重要石油加工过程，同时也副产氢
气、液化气。催化重整工艺中存在催化剂由于结焦失活的情况，需要
烧焦再生，烧焦过程存在CO2 的排放。</p>
            <p>催化重整装置的催化剂烧焦可
能由企业自身进行，也可能由专门进行催化剂再生或回收的其它企业
进行。由企业自身进行的催化剂烧焦过程应计入报告主体的工业生产
过程排放中，如果由其它企业进行，则该部分排放不计入报告主体</p>
            <h3 style={{ marginBottom: '12px' }}>计算说明</h3>
            <p>催化重整装置催化剂烧焦排放计算方法：排放量（tCO₂） = 待再生的催化剂量 × (1 - 再生前催化剂含碳量%) × (再生前催化剂含碳量% / (1 - 再生前催化剂含碳量%) - 再生后催化剂含碳量% / (1 - 再生后催化剂含碳量%)) × 44 / 12 × (碳氧化率%)</p>
            <p>- 待再生的催化剂量单位为 t，保留两位小数</p>
            <p>- 再生前催化剂含碳量单位为 %</p>
            <p>- 再生后催化剂含碳量单位为 %</p>
            <p>- 碳氧化率单位为 %, 默认值为98%</p>
        ...
        <div style={{ marginTop: '24px', backgroundColor: '#fff', padding: '20px', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)', marginBottom: '24px' }}>
          <h3 style={{ marginBottom: '20px', color: '#1890ff', fontWeight: 'bold', fontSize: '18px' }}>添加催化重整装置催化剂烧焦排放记录</h3>

          <div style={{ display: 'flex', justifyContent: 'flex-start', gap: '16px', alignItems: 'end' }}>
            <button
              onClick={addNewReformingUnit}
              style={{
                padding: '8px 16px',
        ...
        <div>
          {reformingUnits.map((unit, index) => (
            <div key={unit.id} style={{ marginBottom: '32px', border: '1px solid #e8e8e8', padding: '16px', borderRadius: '4px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  <h3 style={{ margin: 0, fontSize: '16px', marginRight: '16px' }}>催化重整装置 {index + 1}</h3>
                </div>
                {(reformingUnits.length > 1) && (
                  <button
                    onClick={() => removeReformingUnit(unit.id)}
                    style={{
                      padding: '4px 12px',
...
export default PetroleumCatalyticReformingEmission;