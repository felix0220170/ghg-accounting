...
// 为草酸初始化数据（纵向布局）
const initializeOxalicAcidData = () => {
  const initialData = createInitialIndicatorData(OXALIC_ACID_INDICATORS);
...
function NoneFerrousMetalsProcessEmission({ onEmissionChange, title='' }) {
  // 碳酸盐列表状态
  ...
  // 草酸数据状态
  const [oxalicAcidData, setOxalicAcidData] = useState(null);
  ...
  useEffect(() => {
    ...
    // 初始化草酸数据
    const initialOxalicAcidData = initializeOxalicAcidData();
    setOxalicAcidData(initialOxalicAcidData);
  }, []);
  ...
  const updateCalculatedValues = useCallback(() => {
    ...
    setOxalicAcidData(prevData => {
      ...
      for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
        ...
        // 获取当月的草酸浓度
        const concentrationData = prevData.data.concentration?.find(m => m.month === month);
        const concentrationValue = concentrationData?.value ? parseFloat(concentrationData.value) : 99.6; // 默认值99.6%
        
        // 计算草酸排放因子：0.349 × 草酸的浓度
        const emissionFactorValue = OXALIC_ACID_CONSTANTS.MOLECULAR_RATIO * (concentrationValue / 100);
        
        // 更新排放因子数据
        const currentEmissionFactorData = updatedData.data.emissionFactor || [];
        ...
        updatedData = {
          ...
          data: {
            ...updatedData.data,
            emissionFactor: newEmissionFactorData,
            emission: newEmissionData
          }
        };
      }
  ...
  }, [setCarbonateProducts, setOxalicAcidData]);
  
  // 当数据变化时，更新计算值
  useEffect(() => {
    updateCalculatedValues();
  }, [updateCalculatedValues, carbonateProducts, oxalicAcidData]);
    ...
    if (id === 'oxalic-acid-1') {
      ...
      setOxalicAcidData(prevData => {
        ...
        if (monthIndex !== -1) {
          ...
          hasChanges = true;
          // 获取指标定义以确定单位
          const indicatorDefinition = OXALIC_ACID_INDICATORS.find(ind => ind.key === indicatorKey);
  ...
  const totalEmission = useMemo(() => {
    ...
    // 计算草酸总排放量
    if (oxalicAcidData && oxalicAcidData.data && oxalicAcidData.data['emission']) {
      oxalicAcidData.data['emission'].forEach(monthData => {
        total += parseFloat(monthData.value) || 0;
      });
    }
    
    return total;
  }, [carbonateProducts, oxalicAcidData]);
  ...
  const calculateMonthlyEmissionTotals = () => {
    ...
    for (let month = 0; month < 12; month++) {
      ...
      // 计算草酸排放量
      if (oxalicAcidData && oxalicAcidData.data && oxalicAcidData.data['emission']) {
        const emissionData = oxalicAcidData.data['emission'];
        const monthData = emissionData.find(d => d.month === month + 1);
        const emissionValue = monthData?.value || 0;
        monthTotal += parseFloat(emissionValue) || 0;
      }
  ...
  return (
    <div style={{ padding: '16px' }}>
      ...
      <div style={{ backgroundColor: '#e6f7ff', padding: '12px', borderRadius: '4px', marginBottom: '24px', borderLeft: '4px solid #1890ff' }}>
        <p style={{ margin: 0, color: '#333' }}>过程排放量是企业消耗的各种碳酸盐以及草酸发生分解反应导致的排放量之和。</p>
      </div>
      ...
      <div style={{ marginTop: '20px', marginBottom: '40px' }}>
        <div style={{ marginBottom: '20px' }}>
          <div style={{ backgroundColor: '#f5f5f5', padding: '16px', borderRadius: '4px', lineHeight: '1.2' }}>
            <h3 style={{ marginBottom: '12px' }}>计算说明</h3>
            <p>草酸的排放计算方法：排放量（tCO₂） = 草酸消耗量（t） × 草酸分解的二氧化碳排放因子</p>
            <p>草酸分解的二氧化碳排放因子 = 0.349 × 草酸的浓度（默认值99.6%）</p>
            <p>- 0.349 是二氧化碳与工业草酸的分子量之比</p>
            <p>- 消费量单位为 t，保留两位小数</p>
            <p>- 草酸浓度单位为 %，保留一位小数，默认值为99.6%</p>
            <p>- 排放因子单位为 tCO₂/t，保留四位小数，根据草酸浓度自动计算</p>
            <p>- 排放量单位为 tCO₂，保留两位小数</p>
          </div>
        </div>
        ...
        {oxalicAcidData && (
          <div style={{ backgroundColor: '#fff', padding: '20px', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)', marginBottom: '24px' }}>
            <h3 style={{ marginBottom: '20px', color: '#1890ff', fontWeight: 'bold', fontSize: '18px' }}>草酸分解排放记录</h3>
            {renderVerticalLayoutTable(oxalicAcidData, OXALIC_ACID_INDICATORS)}
          </div>
        )}
      </div>
    </div>
...
export default NoneFerrousMetalsProcessEmission;
