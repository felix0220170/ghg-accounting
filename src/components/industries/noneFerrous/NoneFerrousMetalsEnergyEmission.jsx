...
const DEFAULT_CH4_ESCAPE_PRODUCTS = [
  { id: 'process-emission-1', name: '蓝炭', emissionFactor: 2.853, type: 'solid'},
  { id: 'process-emission-2', name: '焦炭', emissionFactor: 2.862, type: 'solid'},
  { id: 'process-emission-3', name: '无烟煤', emissionFactor: 1.924, type: 'solid'},
  { id: 'process-emission-4', name: '天然气', emissionFactor: 21.622, type: 'gas'},
];
...
function NoneFerrousMetalsEnergyEmission({ onEmissionChange }) {
  // 碳酸盐列表状态
  ...
  // 添加新的自定义碳酸盐
    const addNewCarbonateProduct = useCallback((id) => {
      ...
      const DEFAULT_CARBONATE_PRODUCT = [...customCarbonMaterials].find(product => product.id === id);
      const newProduct = {
        id: `${newProductId}-${Date.now()}`,
        name: DEFAULT_CARBONATE_PRODUCT?.name || '新碳材料',
    ...
    const tableHeaders = (
      <thead>
        <tr style={{ backgroundColor: '#f5f5f5' }}>
          <th style={{ border: '1px solid #d9d9d9', padding: '8px', textAlign: 'center', width: '200px' }}>指标名称</th>
        ...
        {indicators.map(indicator => {
          ...
          return (
              <tr key={indicator.key}>
                <td style={{ border: '1px solid #d9d9d9', padding: '8px' }}>{item.id === DEFAULT_CH4_ESCAPE_PRODUCTS[0].id ? indicator.name : indicator.sub_name}</td>
                <td style={{ border: '1px solid #d9d9d9', padding: '8px', textAlign: 'center' }}>
                  {/* 根据能源类型和指标类型动态显示单位 */}
                  {indicator.key === 'consumptionAmount' && item.type === 'gas' ? (
                    <span>10<sup>4</sup>Nm³</span>
                  ) : indicator.key === 'emissionFactor' && item.type === 'gas' ? (
                    <span>tCO₂/<span>10<sup>4</sup>Nm³</span></span>
                  ) : (
                    indicator.unit
                  )}
                </td>
  ...
  return (
    <div style={{ padding: '16px' }}>
      <h2 style={{ marginBottom: '24px', fontSize: '20px', color: '#333' }}>能源作为原材料用途的二氧化碳排放</h2>
      ...
      <div style={{ marginTop: '20px',marginBottom: '40px' }}>
        <div style={{ marginBottom: '20px' }}>
          <div style={{ marginBottom: '16px', lineHeight: '1.6' }}>
            <p>工业生产中， 能源作为原材料被消耗， 发生化学反应而产生的
温室气体排放。 铜冶炼、 铅锌冶炼等子行业的企业使用焦炭、 蓝炭、
无烟煤、天然气等能源产品作为还原剂，导致二氧化碳排放。</p>
          </div>
          <h3 style={{ marginBottom: '16px' }}>计算说明</h3>
          <div style={{ backgroundColor: '#f5f5f5', padding: '16px', borderRadius: '4px', lineHeight: '1.2' }}>
            <p>计算方法：排放量（tCO₂） = 能源产品作为还原剂的消耗量 × 能源产品作为还原剂用途的二氧化碳排放因子</p>
            <p>- 对固体或液体能源, 单位为 t，对气体能源，单位为万立方米（10<sup>4</sup>Nm³）保留两位小数</p>
            <p>- 排放因子单位为 tCO₂/t还原剂，保留四位小数</p>
        ...
        <div style={{ marginTop: '24px', backgroundColor: '#fff', padding: '20px', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)', marginBottom: '24px' }}>
          <h3 style={{ marginBottom: '20px', color: '#1890ff', fontWeight: 'bold', fontSize: '18px' }}>添加其他能源原材料排放记录</h3>

          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: '16px', alignItems: 'end' }}>
            <div>
              <label htmlFor="carbonateProductType" style={{ display: 'block', marginBottom: '6px', fontWeight: 'bold', color: '#333' }}>添加能源原材料 <span style={{ color: '#ff4d4f' }}>*</span></label>
            <select
              onChange={(e) => {
...
export default NoneFerrousMetalsEnergyEmission;
