...
function OtherCarbonInventory() {
  ...
  const inventoryItems = [
    ...
    { 
      key: '3', 
      name: '碳酸盐使用过程相关资料', 
      description: '石灰石、白云石等碳酸盐原料采购记录、消耗记录、使用用途说明、工艺参数记录', 
      required: true,
      category: '碳酸盐使用过程排放'
    },
    { 
      key: '4', 
      name: '废水厌氧处理排放相关资料', 
      description: '废水量记录、进水和出水COD浓度检测报告、污泥处理记录、厌氧处理工艺参数记录、MCF值选择依据', 
      required: true,
      category: '废水厌氧处理排放'
    },
    { 
      key: '5', 
      name: 'CH₄回收与销毁相关资料', 
      description: 'CH₄回收利用记录、回收自用量和外供量记录、CH₄体积浓度检测报告、火炬销毁装置运行记录、销毁效率数据、甲烷氧化系数数据', 
      required: true,
      category: 'CH₄回收与销毁'
    },
    { 
      key: '6', 
      name: 'CO₂回收利用相关资料', 
      description: 'CO₂回收利用记录、回收自用量和外供量记录、CO₂体积浓度检测报告、作为原料使用记录', 
      required: true,
      category: 'CO₂回收利用'
    },
    { 
  ...
  return (
    <Card title="工业其他行业企业碳核查资料清单" style={{ marginBottom: '20px' }}>
      <Title level={5}>资料清单说明</Title>
      <Paragraph>
        本模块用于上传工业其他行业企业碳核查所需的资料清单文件。适用于那些尚没有针对性的行业企业温室气体核算方法与报告指南的工业其他行业企业。
      </Paragraph>
      <Paragraph>
        <strong>核算范围：</strong>
      </Paragraph>
      ...
      <Paragraph>
        请按照要求上传相关证明材料和记录文档。所有资料为碳核查工作的必要支持文件，用于数据质量控制和审核。重点关注化石燃料使用、碳酸盐使用、废水厌氧处理、CH₄回收与销毁、CO₂回收利用以及购入能源等相关记录，确保碳排放核算的准确性和完整性。
      </Paragraph>
      <Paragraph style={{ fontSize: '12px', color: '#666', marginTop: '10px', padding: '8px', backgroundColor: '#f5f5f5', borderLeft: '3px solid #1890ff' }}>
        <strong>重要说明：</strong>根据工业其他行业企业核算指南，请确保所有排放源数据的完整性和准确性，特别是碳酸盐使用过程排放、废水厌氧处理排放、CH₄回收与销毁量以及CO₂回收利用量的计算。所有计量设备应定期检定，数据记录应规范完整。
      </Paragraph>
...
export default OtherCarbonInventory;
