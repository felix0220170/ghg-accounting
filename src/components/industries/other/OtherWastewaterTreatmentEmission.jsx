...
const TREATMENT_TYPES = {
  OCEAN_RIVER_LAKE: 'OCEAN_RIVER_LAKE',
  AEROBIC_MANAGED: 'AEROBIC_MANAGED',
  AEROBIC_OVERLOADED: 'AEROBIC_OVERLOADED',
  SLUDGE_ANAEROBIC_DIGESTER: 'SLUDGE_ANAEROBIC_DIGESTER',
  ANAEROBIC_REACTOR: 'ANAEROBIC_REACTOR',
  SHALLOW_ANAEROBIC_POND: 'SHALLOW_ANAEROBIC_POND',
  DEEP_ANAEROBIC_POND: 'DEEP_ANAEROBIC_POND'
};

// 定义各处理类型的MCF推荐值和范围
const MCF_RECOMMENDATIONS = {
  [TREATMENT_TYPES.OCEAN_RIVER_LAKE]: {
    name: '海洋、河流或湖泊排放',
    recommended: 0.1,
    range: '0-0.2',
    note: '高浓度有机污水进入河流可能产生厌氧反应'
  },
  [TREATMENT_TYPES.AEROBIC_MANAGED]: {
    name: '好氧处理设施（管理完善）',
    recommended: 0,
    range: '0-0.1',
    note: '必须管理完善'
  },
  [TREATMENT_TYPES.AEROBIC_OVERLOADED]: {
    name: '好氧处理设施（管理不完善，过载）',
    recommended: 0.3,
    range: '0.2-0.4',
    note: ''
  },
  [TREATMENT_TYPES.SLUDGE_ANAEROBIC_DIGESTER]: {
    name: '污泥厌氧消化池',
    recommended: 0.8,
    range: '0.8-1.0',
    note: '未考虑 CH₄ 回收'
  },
  [TREATMENT_TYPES.ANAEROBIC_REACTOR]: {
    name: '厌氧反应器',
    recommended: 0.8,
    range: '0.8-1.0',
    note: '未考虑 CH₄ 回收'
  },
  [TREATMENT_TYPES.SHALLOW_ANAEROBIC_POND]: {
    name: '浅厌氧塘',
    recommended: 0.2,
    range: '0-0.3',
    note: '深度不足 2 米'
  },
  [TREATMENT_TYPES.DEEP_ANAEROBIC_POND]: {
    name: '深厌氧塘',
    recommended: 0.8,
    range: '0.8-1.0',
    note: '深度超过 2 米'
...
const DEFAULT_TREATMENT_TYPE = TREATMENT_TYPES.AEROBIC_MANAGED; // 默认处理类型
...
// 创建初始的指标数据（纵向布局）
const createInitialIndicatorData = (treatmentType = DEFAULT_TREATMENT_TYPE) => {
  ...
  return indicators.reduce((acc, indicator) => {
    ...
    // 根据处理类型调整MCF的默认值
    if (indicator.key === 'mcf' && MCF_RECOMMENDATIONS[treatmentType]) {
      defaultValue = MCF_RECOMMENDATIONS[treatmentType].recommended;
    }
    
    acc[indicator.key] = MONTHS.map((month, index) => ({
      month: index + 1,
      monthName: month,
      value: defaultValue !== undefined ? defaultValue : (indicator.isCalculated ? 0 : ''),
      unit: indicator.unit
    }));
    return acc;
  }, {});
...
// 为废水处理初始化数据（纵向布局）
const initializeWastewaterRecordData = (treatmentType = DEFAULT_TREATMENT_TYPE) => {
  const initialData = createInitialIndicatorData(treatmentType);
...
function FoodWastewaterTreatmentEmission({ onEmissionChange, title='', isPaper = false }) {
  ...
  // 处理类型选择状态
  const [selectedTreatmentType, setSelectedTreatmentType] = useState(isPaper ? "PAPER" : DEFAULT_TREATMENT_TYPE);
  ...
  const handleTreatmentTypeChange = (e) => {
    const treatmentType = e.target.value;
  ...
  const handleMcfUsageModeChange = (e) => {
    ...
    if (mode === 'recommended' && MCF_RECOMMENDATIONS[selectedTreatmentType]) {
      const recommendedValue = MCF_RECOMMENDATIONS[selectedTreatmentType].recommended;
      setCustomMcf('');
  ...
  // 初始化默认数据
  useEffect(() => {
    const initializedRecord = initializeWastewaterRecordData(selectedTreatmentType);
    setWastewaterRecords([initializedRecord]);
  }, [selectedTreatmentType]);
  
  // 添加新的废水处理记录
  const addNewWastewaterRecord = useCallback(() => {
    const newRecord = initializeWastewaterRecordData(selectedTreatmentType);
  ...
  }, [selectedTreatmentType, mcfUsageMode, customMcf]);
    ...
    setWastewaterRecords(prevRecords => {
      ...
      const updatedRecords = prevRecords.map(record => {
        ...
        for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
          ...
          if (actualRemovedCOD === undefined || actualRemovedCOD === null || actualRemovedCOD === '') {
            if (wastewaterVolume && inletCOD && outletCOD) {
  ...
  const handleDataChange = useCallback((id, indicatorKey, month, field, value) => {
    // 不进行格式化，直接使用原始输入值，但要处理 undefined 和 null
          ...
          return (
              <tr key={indicator.key}>
                ...
                {MONTHS.map((month, index) => {
                  ...
                  const value = monthData?.value !== undefined ? monthData.value : '';
      ...
      <div style={{ marginTop: '20px',marginBottom: '40px' }}>
        <div style={{ marginBottom: '20px' }}>
          <div style={{ backgroundColor: '#f5f5f5', padding: '16px', borderRadius: '4px', lineHeight: '1.2' }}>
            ...
            <p>废水厌氧处理CH₄排放计算方法：</p>
            <p>CH₄ 排放量（kg CH₄） = (厌氧处理系统去除的 COD 量 - 以污泥方式清除掉的 COD 量) × 甲烷最大生产能力 × 甲烷修正因子 (MCF)</p>
        ...
        <div style={{ marginTop: '24px', backgroundColor: '#fff', padding: '20px', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)', marginBottom: '24px' }}>
          ...
          <div style={{ marginBottom: '20px' }}>
            <label style={{ display: 'block', marginBottom: '8px', fontWeight: 'bold' }}>选择处理和排放途径或系统类型：</label>
            <select
            ...
            >
              <option value={TREATMENT_TYPES.OCEAN_RIVER_LAKE}>海洋、河流或湖泊排放</option>
              <option value={TREATMENT_TYPES.AEROBIC_MANAGED}>好氧处理设施（管理完善）</option>
              <option value={TREATMENT_TYPES.AEROBIC_OVERLOADED}>好氧处理设施（管理不完善，过载）</option>
              <option value={TREATMENT_TYPES.SLUDGE_ANAEROBIC_DIGESTER}>污泥厌氧消化池</option>
              <option value={TREATMENT_TYPES.ANAEROBIC_REACTOR}>厌氧反应器</option>
              <option value={TREATMENT_TYPES.SHALLOW_ANAEROBIC_POND}>浅厌氧塘</option>
              <option value={TREATMENT_TYPES.DEEP_ANAEROBIC_POND}>深厌氧塘</option>
            </select>
          </div>
          ...
          <div style={{ marginBottom: '20px', padding: '12px', backgroundColor: '#f5f5f5', borderRadius: '4px' }}>
            ...
            {selectedTreatmentType && MCF_RECOMMENDATIONS[selectedTreatmentType] && mcfUsageMode === 'recommended' && (
              <div>
                <p style={{ margin: '4px 0' }}><strong>处理类型：</strong>{MCF_RECOMMENDATIONS[selectedTreatmentType].name}</p>
                <p style={{ margin: '4px 0' }}><strong>建议MCF值：</strong>{MCF_RECOMMENDATIONS[selectedTreatmentType].recommended}</p>
                <p style={{ margin: '4px 0' }}><strong>MCF范围：</strong>{MCF_RECOMMENDATIONS[selectedTreatmentType].range}</p>
                {MCF_RECOMMENDATIONS[selectedTreatmentType].note && (
                  <p style={{ margin: '4px 0' }}><strong>备注：</strong>{MCF_RECOMMENDATIONS[selectedTreatmentType].note}</p>
                )}
              </div>
            )}
