...
// 矿井类型
const MINE_TYPES = [
  { id: 'continuous', name: '已实现CO2 连续监测的矿井' },
  { id: 'discontinuous', name: '尚未实现CO2 连续监测的矿井' }
];

// 矿井排放指标（已实现连续监测）
const CONTINUOUS_MINE_INDICATORS = [
  {
    key: 'co2VentilationAmount',
    name: 'CO2 风排量',
    unit: '万Nm³',
    isCalculated: false,
    decimalPlaces: 2
...
const DISCONTINUOUS_MINE_INDICATORS = [
  {
    key: 'avgCo2VentilationPerMinute',
    name: '当月平均每分钟CO2风排量',
    unit: 'Nm³/min',
    isCalculated: false,
    decimalPlaces: 2
...
const SHARED_EMISSION_INDICATORS = [
  {
    key: 'co2RecoveryUtilizationAmount',
    name: 'CO2 回收利用量',
    unit: '万Nm³',
    isCalculated: false,
    decimalPlaces: 2
...
const EMISSION_CONSTANTS = {
  CO2_DENSITY: 19.7, // 标准状况下CO2的密度，单位为吨CO2/万Nm³
  CO2_GWP: 1 // CO2的全球变暖潜能值
};
...
function CoalMineCO2Emission({ onEmissionChange }) {
  // 矿井记录列表状态
    ...
    setMineRecords(prevRecords => {
      ...
      const updatedRecords = prevRecords.map(mine => {
        ...
        for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
          ...
          // 获取当月的平均每分钟CO2风排量和工作日数
          const avgCo2VentilationData = mine.data.avgCo2VentilationPerMinute?.find(m => m.month === month);
          const avgCo2VentilationValue = avgCo2VentilationData?.value ? parseFloat(avgCo2VentilationData.value) : 0;
          ...
          // 计算CO2风排量：平均每分钟风排量 × 工作日数 × 60 × 24 × 0.0001
          const co2VentilationAmountValue = avgCo2VentilationValue * workDaysValue * 60 * 24 * 0.0001;
          
          // 更新CO2风排量数据
          const currentCo2VentilationData = updatedMine.data.co2VentilationAmount || [];
          const co2VentilationMonthIndex = currentCo2VentilationData.findIndex(m => m.month === month);
          const newCo2VentilationData = [...currentCo2VentilationData];
          
          if (co2VentilationMonthIndex !== -1) {
            if (parseFloat(newCo2VentilationData[co2VentilationMonthIndex].value) !== co2VentilationAmountValue) {
              newCo2VentilationData[co2VentilationMonthIndex] = {
                ...newCo2VentilationData[co2VentilationMonthIndex],
                value: co2VentilationAmountValue,
                unit: '万Nm³'
              };
              mineChanged = true;
            }
          } else if (co2VentilationAmountValue > 0) {
            // 确保值大于0时才添加新数据
            newCo2VentilationData.push({
              month,
          ...
          updatedMine = {
            ...
            data: {
              ...updatedMine.data,
              co2VentilationAmount: newCo2VentilationData
            }
          };
        }
  ...
  const calculateMonthlyEmissionTotals = useCallback(() => {
    ...
    for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
      ...
      mineRecords.forEach(mine => {
        ...
        // 获取当月的CO2风排量和抽放量
        const co2VentilationData = mine.data.co2VentilationAmount?.find(m => m.month === month);
        const co2VentilationValue = co2VentilationData?.value ? parseFloat(co2VentilationData.value) : 0;

        const co2ExtractionData = mine.data.co2ExtractionAmount?.find(m => m.month === month);
        const co2ExtractionValue = co2ExtractionData?.value ? parseFloat(co2ExtractionData.value) : 0;
        
        // 累加矿井的CO2风排量和抽放量
        monthTotal += co2VentilationValue + co2ExtractionValue;
      });
      
      // 减去CO2回收利用量
       if (sharedEmissionData) {
         // 获取当月的CO2回收利用量
         const recoveryData = sharedEmissionData.data?.co2RecoveryUtilizationAmount?.find(m => m.month === month);
         const recoveryValue = recoveryData?.value ? parseFloat(recoveryData.value) : 0;
         
         // 减去CO2回收利用量
         monthTotal -= recoveryValue;
       }
      
      // 计算最终排放量：(风排量+抽放量-回收利用量) * CO2密度
      monthTotal = Math.max(0, monthTotal) * EMISSION_CONSTANTS.CO2_DENSITY;
  ...
  const renderTotalEmissionTable = () => {
    ...
    return (
      <table style={{ borderCollapse: 'collapse', width: '100%', marginTop: '24px', backgroundColor: '#f9f9f9' }}>
        ...
        <tbody>
          <tr>
            <td style={{ border: '1px solid #d9d9d9', padding: '8px', fontWeight: 'bold' }}>井工开采的CO2逃逸排放总量</td>
  ...
  return (
    <div style={{ padding: '16px' }}>
      <h2 style={{ marginBottom: '24px', fontSize: '20px', color: '#333' }}>井工开采的CO2逃逸排放</h2>
      {/* 井工开采的CO2逃逸排放 */}
      <div style={{ marginTop: '20px',marginBottom: '40px' }}>
        <div style={{ marginBottom: '20px' }}>
          <div style={{ backgroundColor: '#f5f5f5', padding: '16px', borderRadius: '4px', lineHeight: '1.2' }}>
            <h3 style={{ marginBottom: '24px' }}>计算说明</h3>
            <p>井工开采过程中，CO2从煤矿巷道和采空区逃逸排放，排放量根据矿井类型（连续监测或非连续监测）分别计算。</p>
            <p>已实现CO2 连续监测的矿井需直接输入CO2风排量和CO2抽放量。</p>
            <p>尚未实现CO2 连续监测：CO2风排量 = 当月平均每分钟CO2风排量 × 当月实际工作日数 × 60 × 24 × 0.0001（单位：万Nm³）</p>
            <p>总排放 = （各矿井的CO2风排量之和 + 各矿井的CO2抽放量 - CO2的回收利用量） × 19.7</p>
            <p>其中：19.7是标准状况下CO2的密度（吨CO2/10<sup>4</sup>Nm³）</p>
            <p>- CO2风排量、抽放量、回收利用量单位均为 10<sup>4</sup>Nm³</p>
            <p>- 排放量单位为 tCO₂</p>
          </div>
        </div>
...
export default CoalMineCO2Emission;
