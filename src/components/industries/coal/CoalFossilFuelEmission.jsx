...
// 煤行业化石燃料燃烧排放量组件（工序驱动）
function CoalFossilFuelEmission({ onEmissionChange, productionLines = [], onProductionLinesChange }) {
  ...
  const addNewProcess = useCallback(() => {
    if (!newProcessName.trim() || !onProductionLinesChange || !Array.isArray(processes)) return;
    ...
    setNewProcessName(''); // 清空输入框
  }, [newProcessName, processes, onProductionLinesChange]);
  ...
  const calculateMonthlyEmission = useCallback((monthData) => {
    ...
    if (receivedBaseCarbonContent > 0) {
      return consumption * receivedBaseCarbonContent * (oxidationRate / 100) * CARBON_TO_CO2_RATIO;
    } else {
          ...
          process.fuelItems.forEach(fuelItem => {
            ...
            if (fuelItem && Array.isArray(fuelItem.monthlyData)) {
              fuelItem.monthlyData.forEach((monthData, index) => {
                const monthEmission = calculateMonthlyEmission(monthData);
  ...
  }, [processes, calculateMonthlyEmission, otherMobileFuels]);
  ...
  return (
    <div className="fossil-fuel-emission">
      ...
      <div className="calculation-description" style={{ marginBottom: '20px', padding: '15px', border: '1px solid #e0e0e0', borderRadius: '6px', backgroundColor: '#fafafa' }}>
        ...
        <p>指化石燃料在各种类型的固定或移动燃烧设备中（如锅炉、燃烧器、涡轮机、加热器、焚烧炉、煅烧炉、窑炉、内燃机等）与氧气充分燃烧生成的CO2 排放</p>
        <p><strong>计算公式：</strong></p>
        <p>1. 重点燃烧设施CO2排放量 = Σ(各化石燃料消耗量 × （化石燃料收到基元素碳含量[如有] 或 （化石燃料收到基低位发热量 × 化石燃料单位热值含碳量）） × 化石燃料碳氧化率 × 44/12)</p>
        <p>2. 其他燃烧设施的CO2排放量 = Σ(其他部分各化石燃料消耗量 × （化石燃料收到基元素碳含量[如有] 或 （化石燃料收到基低位发热量 × 化石燃料单位热值含碳量）） × 化石燃料碳氧化率 × 44/12)</p>
        <p><strong>注：</strong>收到基元素碳含量 = 收到基低位发热量 × 单位热值含碳量。如用户输入了收到基元素碳含量，则优先使用该值进行计算；否则，使用收到基低位发热量和单位热值含碳量的乘积进行计算。</p>
        <p>3. 企业总CO2排放量 = 固定设备CO2排放量之和 + 其他部分CO2排放量</p>
        <p>其中：44/12 是二氧化碳与碳的相对分子质量之比</p>
      </div>  

      {/* 添加重点燃烧设施区域 */}
      <div style={{ marginBottom: '20px', padding: '15px', border: '1px solid #e8e8e8', borderRadius: '4px', backgroundColor: '#f0f7ff' }}>
        <h3 style={{ marginBottom: '15px', color: '#2196F3' }}>添加重点燃烧设施</h3>
        <p style={{ marginBottom: '10px', color: '#666', fontSize: '14px', fontStyle: 'italic' }}><strong>注：</strong>年排放达1 万吨CO2 及以上的燃烧设施为重点燃烧设施, 应为每个重点燃烧设施分别填写</p>
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
          <input
            type="text"
            placeholder="请输入重点燃烧设施名称"
            value={newProcessName}
            onChange={(e) => setNewProcessName(e.target.value)}
            style={{ padding: '8px 12px', border: '1px solid #d9d9d9', borderRadius: '4px', width: '300px' }}
          />
          <button
            ...
            style={{
              ...
              border: 'none',
              borderRadius: '4px',
              cursor: newProcessName.trim() ? 'pointer' : 'not-allowed',
              opacity: newProcessName.trim() ? 1 : 0.6
            }}
          >
            添加重点燃烧设施
          </button>
      ...
      {Array.isArray(processes) && processes.map((process, processIndex) => (
        <div key={process.id} className="process-section" style={{ marginBottom: '30px', padding: '20px', border: '2px solid #4CAF50', borderRadius: '8px', backgroundColor: '#f9fff9' }}>
          <div className="process-header" style={{ display: 'flex', alignItems: 'center', marginBottom: '20px' }}>
            <h3 style={{ margin: '10px 0' }}>{process.processName}{processIndex + 1}</h3>
          </div>
      ...
      <div key="other-mobile" className="production-line" style={{ marginBottom: '30px', padding: '20px', border: '2px solid #2196F3', borderRadius: '8px', backgroundColor: '#f0f7ff' }}>
        <div className="line-header" style={{ display: 'flex', alignItems: 'center', marginBottom: '20px' }}>
          <h3 style={{ margin: '10px 0' }}>其他燃烧设备</h3>
        </div>
...
export default CoalFossilFuelEmission;
