...
const AC_CONSUMPTION_INDICATORS = [
  {
    ...
    description: '用户输入值，表示铝电解工序交流电耗量'
  }
];
...
function AluminumACConsumption({ onEmissionChange, productionLines = [], onProductionLinesChange }) {
  ...
  // 辅助函数：更新流程中的指标值
  const updateProcessIndicator = (process, indicatorKey, month, value, unit = 'MW∙h') => {
  ...
  return (
    <div>
      <h2 style={{ marginBottom: '20px' }}>辅助参数报告项 1：铝电解工序交流电耗</h2>
        ...
        <div className="calculation-section" style={{ marginTop: '20px' }}>
          ...
          <div className="table-container" style={{ overflowX: 'auto', marginBottom: '10px' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', marginTop: '10px', minWidth: '1200px' }}>
              ...
              <tbody>
                <tr>
                  <td style={{ border: '1px solid #d9d9d9', padding: '8px', textAlign: 'center', fontWeight: 'bold' }}>交流电耗总量 (MW·h)</td>
                  {(() => {
                    // 计算月度和年度电耗总量
                    const calculateMonthlyConsumptionTotals = () => {
                    ...
                    // 调用计算函数获取月度和年度电耗总量
                    const { monthlyTotals, yearlyTotal } = calculateMonthlyConsumptionTotals();
