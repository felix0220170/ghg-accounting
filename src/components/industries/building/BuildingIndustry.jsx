...
const { TabPane } = Tabs;
import IndustrySummary from './BuildingIndustrySummary';
import NetElectricityHeatEmission from '../common/NetElectricityHeatEmission';
import CarbonInventory from './BuildingCarbonInventory';
import FossilFuelEmission from '../chemical/ChemicalFossilFuelEmission';

function BuildingIndustry({ onEmissionChange }) {
  ...
  return (
    <div className="building-industry">
      <Card title="公共建筑运营单位（企业）温室气体排放核算" style={{ marginBottom: '20px' }}>
        <Title level={4}>行业说明</Title>
        <Paragraph>
          适用于中国公共建筑，包括办公建筑(写字楼、政府部门办公楼等)，商业建筑(商场、金融建筑等)，旅游建筑(旅馆酒店、娱乐场所等)，科教文卫建筑(包括文化、教育、科研、医
疗、卫生、体育建筑等)，通信建筑(邮电、通讯、广播用房)以及交通运输用房(机场、车站建筑等)等的运营单位（企业）等。
        </Paragraph>
        <Paragraph>
          核算范围包括：化石燃料燃烧排放、净购入电力和热力隐含的CO2排放。
        </Paragraph>
        <Paragraph>
          <strong>边界说明：</strong>根据相关指南，以下排放源一般不纳入核算范围：
        </Paragraph>
        <ul style={{ marginLeft: '20px', marginTop: '10px' }}>
          <li>逸散型排放源（如冰箱、空调、灭火器和化粪池等产生的排放），由于排放数量较小，一般不予考虑</li>
          <li>建筑物周围新种植树木的温室气体抵消，由于抵消数量较小，一般不予考虑</li>
          <li>委托第三方承担运输产生的排放，统计复杂且容易重复计算，一般不考虑</li>
        </ul>
      </Card>
...
export default BuildingIndustry;
