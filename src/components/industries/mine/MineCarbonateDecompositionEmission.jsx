...
function MineCarbonateDecompositionEmission({ onEmissionChange, title='' }) {
  ...
  const addNewCarbonateProduct = useCallback((id) => {
    ...
    const newProduct = {
      ...
      name: DEFAULT_CARBONATE_PRODUCT?.name || '新矿石',
    ...
    setCarbonateProducts(prevProducts => {
      ...
      const updatedProducts = prevProducts.map(product => {
        ...
        for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
          ...
          // 计算排放量：矿石煅烧量 * 分解率（转换为小数） * 排放因子
          const emissionValue = Math.max(0, consumptionAmountValue) * (decompositionRateValue / 100) * emissionFactorValue;
      ...
      <div style={{ marginTop: '20px',marginBottom: '40px' }}>
        <div style={{ marginBottom: '20px' }}>
          <div style={{ backgroundColor: '#f5f5f5', padding: '16px', borderRadius: '4px', lineHeight: '1.2' }}>
            ...
            <p>碳酸盐分解的CO₂排放量可根据矿石的焙烧或煅烧量、煅烧或焙烧的分解率和矿石中碳酸盐组分的质量分数及其排放因子计算</p>
            <p>碳酸盐分解排放计算方法：排放量（tCO₂） = 矿石的煅烧或焙烧量（t） × 矿石的分解率（%） × 排放因子（tCO₂/t）</p>
            <p>排放因子计算方法：排放因子（tCO₂/t） = Σ(各碳酸盐排放因子 × 相应的质量分数)</p>
            <p>- 矿石的煅烧或焙烧量单位为 t，保留两位小数</p>
            <p>- 矿石的分解率单位为 %，保留整数</p>
        ...
        >
          ...
          >
            <Form.Item
              name="productName"
              label="矿石名称"
              rules={[{ required: true, message: '请输入矿石名称' }]}
            >
              <Input
                placeholder="请输入矿石名称"
            ...
            <div key={product.id} style={{ marginBottom: '32px', border: '1px solid #e8e8e8', padding: '16px', borderRadius: '4px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '16px' }}>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <h3 style={{ margin: 0, fontSize: '16px', marginBottom: '4px' }}>矿石({index + 1}):  {product.name}</h3>
