...
function MineCarbonationCO2Absorption({ onEmissionChange, title='' }) {
  // 碳酸盐列表状态
  ...
  // 获取所有可用的碳酸盐（包括默认和自定义）
  const allCarbonateProducts = useMemo(() => {
    return [...DEFAULT_CARBONATE_PRODUCTS, ...customCarbonMaterials];
  ...
  const addNewCarbonateProduct = useCallback((id) => {
    ...
    const newProduct = {
      ...
      name: DEFAULT_CARBONATE_PRODUCT?.name || '新碳化产物',
  ...
  const renderTotalEmissionTable = () => {
    ...
    return (
      <table style={{ borderCollapse: 'collapse', width: '100%', marginTop: '24px', backgroundColor: '#f9f9f9' }}>
        ...
        <tbody>
          <tr>
            <td style={{ border: '1px solid #d9d9d9', padding: '8px', fontWeight: 'bold' }}>碳化产物吸收的CO₂总量</td>
  ...
  return (
    <div style={{ padding: '16px' }}>
      <h2 style={{ marginBottom: '24px', fontSize: '20px', color: '#333' }}>碳化工艺吸收的CO₂量</h2>
      ...
      <div style={{ marginTop: '20px',marginBottom: '40px' }}>
        <div style={{ marginBottom: '20px' }}>
          <div style={{ backgroundColor: '#f5f5f5', padding: '16px', borderRadius: '4px', lineHeight: '1.2' }}>
            ...
            <h3 style={{ marginBottom: '12px' }}>计算说明</h3>
            <p>碳化产物吸收CO₂量计算方法：吸收量（tCO₂） = 碳化产物质量（t） × 排放因子（tCO₂/t）</p>
            <p>排放因子计算方法：排放因子（tCO₂/t） = Σ(各碳酸盐排放因子 × 相应的质量分数)</p>
            <p>- 碳化产物质量单位为 t，保留两位小数</p>
            <p>- 排放因子单位为 tCO₂/t，保留四位小数</p>
            <p>- 质量分数取值范围为 0-1（无单位）</p>
            <p>- 吸收的CO₂量单位为 tCO₂，保留两位小数</p>
          </div>
        </div>
        ...
        <div style={{ marginTop: '24px', backgroundColor: '#fff', padding: '20px', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)', marginBottom: '24px' }}>
          <h3 style={{ marginBottom: '20px', color: '#1890ff', fontWeight: 'bold', fontSize: '18px' }}>添加碳化产物吸收CO₂记录</h3>
        ...
        >
          ...
          >
            <Form.Item
              name="productName"
              label="碳化产物名称"
              rules={[{ required: true, message: '请输入碳化产物名称' }]}
            >
              <Input
            ...
            <div style={{ marginBottom: '16px' }}>
              <h4 style={{ marginBottom: '8px' }}>添加碳酸盐成分</h4>
              <Select
                style={{ width: '100%', marginBottom: '8px' }}
                placeholder="选择碳酸盐"
                onChange={addCarbonateComponent}
              >
                {allCarbonateProducts.map(product => (
                  <Option key={product.id} value={product.id}>
                    {product.name} (排放因子: {product.emissionFactor} tCO₂/t)
                  </Option>
                ))}
              </Select>
            </div>
            ...
            {selectedComponents.length > 0 && (
              <div style={{ marginBottom: '16px' }}>
                <h4 style={{ marginBottom: '8px' }}>碳酸盐成分列表</h4>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead>
                    <tr style={{ backgroundColor: '#f5f5f5' }}>
                      <th style={{ padding: '8px', border: '1px solid #d9d9d9' }}>名称</th>
                      <th style={{ padding: '8px', border: '1px solid #d9d9d9' }}>排放因子 (tCO₂/t)</th>
                      <th style={{ padding: '8px', border: '1px solid #d9d9d9' }}>质量分数 (0-1)</th>
                  ...
                  <tbody>
                    {selectedComponents.map(component => (
                      <tr key={component.id}>
                        <td style={{ padding: '8px', border: '1px solid #d9d9d9' }}>{component.name}</td>
                        <td style={{ padding: '8px', border: '1px solid #d9d9d9' }}>{component.emissionFactor}</td>
                        <td style={{ padding: '8px', border: '1px solid #d9d9d9' }}>
                          <InputNumber
                            ...
                            step={0.01}
                            precision={4}
                            value={component.massFraction}
                            onChange={(value) => updateMassFraction(component.id, value)}
                          />
                        </td>
                        <td style={{ padding: '8px', border: '1px solid #d9d9d9' }}>
                          <Button
                            type="text"
                            danger
                            onClick={() => removeCarbonateComponent(component.id)}
                          >
                            删除
                          </Button>
                        </td>
            ...
            <div style={{ marginBottom: '16px', padding: '12px', backgroundColor: '#f5f5f5', borderRadius: '4px' }}>
              <h4 style={{ marginBottom: '8px' }}>排放因子计算结果</h4>
              <p style={{ fontSize: '16px', fontWeight: 'bold' }}>
                排放因子: {emissionFactor.toFixed(4)} tCO₂/t
              </p>
              <p style={{ fontSize: '12px', color: '#666' }}>
                计算公式: 排放因子 = Σ(各碳酸盐排放因子 × 相应的质量分数)
              </p>
            </div>
          </Form>
        </Modal>
            ...
            <div key={product.id} style={{ marginBottom: '32px', border: '1px solid #e8e8e8', padding: '16px', borderRadius: '4px' }}>
              <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '16px' }}>
                <div style={{ display: 'flex', flexDirection: 'column' }}>
                  <h3 style={{ margin: 0, fontSize: '16px', marginBottom: '4px' }}>碳化产物({index + 1}):  {product.name}</h3>
...
export default MineCarbonationCO2Absorption;
