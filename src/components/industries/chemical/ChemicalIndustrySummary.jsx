...
const ChemicalIndustrySummary = ({ emissionData, industry, onDataChange }) => {
  // 确保data不为undefined
  ...
  const calculateBaseTotal = () => {
    return (
      ...
      calculateEmission('processEmission') + // 工业生产过程中原材料消耗产生的排放
      calculateEmission('carbonateDecompositionEmission') + // 工业生产过程中碳酸盐分解排放
      calculateEmission('nitricAcidEmission') + // 工业生产过程中硝酸产生的排放
      calculateEmission('adipicAcidEmission') + // 工业生产过程中己二酸产生的排放
      - calculateEmission('recyclingEmission') // 回收利用量需要减去
    );
  };
  ...
  const tableData = [
    ...
    },
    {
      key: 'nitric-acid',
      name: '工业生产过程中硝酸产生的排放',
      value: safeData.nitricAcidEmission || 0,
      co2e: calculateCO2e('nitricAcidEmission', safeData.nitricAcidEmission || 0)
    },
    {
      key: 'adipic-acid',
      name: '工业生产过程中己二酸产生的排放',
      value: safeData.adipicAcidEmission || 0,
      co2e: calculateCO2e('adipicAcidEmission', safeData.adipicAcidEmission || 0)
    },
    {
      key: 'recycling',
      name: '企业CO2回收利用量',
      value: safeData.recyclingEmission || 0,
      co2e: calculateCO2e('recyclingEmission', safeData.recyclingEmission || 0)
    },
    {
  ...
  return (
    <div className="summary-table-container">
      <div className="formula-section" style={{ marginBottom: '20px', padding: '15px', backgroundColor: '#f5f5f5', borderRadius: '8px' }}>
        ...
        <p><strong>工业生产过程总排放</strong> = 工业生产过程中原材料消耗产生的排放 + 工业生产过程中碳酸盐分解排放 + 工业生产过程中硝酸产生的排放 + 工业生产过程中己二酸产生的排放</p>
        <p><strong>基础排放量（不含电力和热力）</strong> = 化石燃料燃烧CO2排放 + 工业生产过程总排放 - <span style={{ color: '#1890ff', fontWeight: 'bold', backgroundColor: '#e6f7ff', padding: '2px 4px', borderRadius: '3px' }}>企业CO2回收利用量</span></p>
        <p><strong>总排放量（含电力和热力）</strong> = 基础排放量（不含电力和热力） + 净购入电力和热力隐含的CO2排放</p>
        <p>注：企业CO2回收利用量作为减排项从总排放量中扣除</p>
      </div>
...
export default ChemicalIndustrySummary;
