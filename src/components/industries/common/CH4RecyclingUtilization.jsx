...
function CH4RecyclingUtilization({ onEmissionChange }) {
    ...
    setCH4RecyclingProducts(prevProducts => {
      ...
      const updatedProducts = prevProducts.map(product => {
        ...
        for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
          ...
          // 计算排放量：(回收外供量 * 外供气体CH4 体积浓度 + 回收作原料量 * 原料气CH4 体积浓度) / 100 * 7.17 * 21
          const emissionValue = Math.max(0, recyclingSupplyAmountValue * supplyGasCH4ConcentrationValue + recyclingRawMaterialAmountValue * rawMaterialGasCH4ConcentrationValue) / 100 * 7.17 * 21;
  ...
  }, [setCH4RecyclingProducts]);
  
  // 当数据变化时，更新计算值
  useEffect(() => {
    updateCalculatedValues();
  }, [updateCalculatedValues, ch4RecyclingProducts]);
  ...
  const handleDataChange = useCallback((id, indicatorKey, month, field, value, type = 'ch4-recycling-product') => {
    ...
    // 使用CH4回收利用相关设置
    const setState = setCH4RecyclingProducts;
    const indicators = CH4_RECYCLING_INDICATORS;
      ...
      const updatedItems = prevItems.map(item => {
        if (item.id === id) {
          ...
          if (monthIndex !== -1) {
            ...
            // 如果月份数据不存在，创建它
            hasChanges = true;
            const indicatorDefinition = CH4_RECYCLING_INDICATORS.find(ind => ind.key === indicatorKey);
  ...
  return (
    <div style={{ padding: '16px' }}>
      <h2 style={{ marginBottom: '24px', fontSize: '20px', color: '#333' }}>企业CH4回收利用量(公用组件)</h2>
      ...
      <div style={{ marginTop: '20px',marginBottom: '40px' }}>
        <div style={{ marginBottom: '20px' }}>
          <h3 style={{ marginBottom: '16px' }}>CH4回收利用计算说明</h3>
          <div style={{ backgroundColor: '#f5f5f5', padding: '16px', borderRadius: '4px', lineHeight: '1.6' }}>
            <p>CH4回收利用量计算方法：回收利用量（tCO₂e） = （CH4回收外供量（tCH₄）× 外供气体CH4体积浓度（%） + CH4回收作原料量（tCH₄）× 原料气CH4体积浓度（%）） / 100 × 7.17 × 21</p>
            <p>- CH4回收外供量单位为 tCH₄，保留两位小数</p>
            <p>- CH4回收作原料量单位为 tCH₄，保留两位小数</p>
            <p>- 气体CH4体积浓度单位为 %，保留整数</p>
            <p>- 回收利用量单位为 tCO₂e，保留两位小数</p>
            <p>- 7.17 为标况下CH4气体的密度，单位为tCH₄/10<sup>4</sup>Nm³</p>
            <p>- 21 为CH4的全球变暖潜能值（GWP）</p>
          </div>
        </div>
