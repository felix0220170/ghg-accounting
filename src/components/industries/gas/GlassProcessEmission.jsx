...
const DEFAULT_CH4_ESCAPE_PRODUCTS = [
  { id: 'process-emission-1', name: '碳粉氧化', emissionFactor: 100},
];
...
const createInitialIndicatorData = () => {
  ...
  return indicators.reduce((acc, indicator) => {
    acc[indicator.key] = MONTHS.map((month, index) => ({
      month: index + 1,
      monthName: month,
      value: indicator.isCalculated ? 0 : (indicator.key === 'emissionFactor' ? 100 : ''),
      unit: indicator.unit
    }));
    return acc;
  }, {});
...
function GlassProcessEmission({ onEmissionChange }) {
  // 碳酸盐列表状态
    ...
    setCarbonateProducts(prevProducts => {
      ...
      const updatedProducts = prevProducts.map(product => {
        ...
        for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
          ...
          // 计算排放量：原料配料中碳粉消耗量 × 碳粉含碳量的加权平均值 × 44/12
          // 注意：含碳量是百分比，需要除以100转换为小数
          const emissionValue = Math.max(0, consumptionAmountValue) * (emissionFactorValue / 100) * (44/12);
  ...
  return (
    <div style={{ padding: '16px' }}>
      <h2 style={{ marginBottom: '24px', fontSize: '20px', color: '#333' }}>原料配料中碳粉氧化的排放</h2>
      ...
      <div style={{ marginTop: '20px',marginBottom: '40px' }}>
        <div style={{ marginBottom: '20px' }}>
          <div style={{ marginBottom: '16px', lineHeight: '1.6' }}>
            <p>平板玻璃生产过程中在原料配料中掺加一定量的碳粉作为还原剂，以降低芒硝的分解温度，促使硫酸钠在低于其熔点温度下快速分解还原，有助于原料的快速升温和熔融，而碳粉中的碳则被氧化为CO2。</p>
          </div>
          <h3 style={{ marginBottom: '16px' }}>计算说明</h3>
          <div style={{ backgroundColor: '#f5f5f5', padding: '16px', borderRadius: '4px', lineHeight: '1.2' }}>
            <p>计算方法：排放量（tCO₂） = 原料配料中碳粉消耗量（t） × 碳粉含碳量的加权平均值（默认为100%）× 44/12</p>
            <p>- 碳粉消耗量单位为 t，保留两位小数</p>
            <p>- 含碳量单位为 %，保留两位小数</p>
...
export default GlassProcessEmission;
