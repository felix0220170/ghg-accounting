...
const GAS_WELL_INDICATORS = [
  ...
  },
  {
    ...
    decimalPlaces: 2,
    getValue: (data) => {
      const { openFlowRate, operatingHours, ch4Concentration } = data;
      return (openFlowRate * operatingHours * (ch4Concentration / 100) * 7.17 * 21).toFixed(2);
    }
  }
];
...
const createInitialIndicatorData = () => {
  const indicators = GAS_WELL_INDICATORS;
...
// 为天然气井初始化数据（纵向布局）
const initializeGasWellData = (wellNumber) => {
  ...
  return {
    id: `gas-well-${Date.now()}`,
    wellNumber: wellNumber,
    name: `天然气井 ${wellNumber}`,
    data: initialData,
    files: {}
  };
};

function GasExplorationCH4Emission({ onEmissionChange }) {
  // 天然气井列表状态
  ...
  // 添加新的天然气井
  const addNewGasWell = useCallback(() => {
    const newWellNumber = gasWells.length + 1;
    const newGasWell = initializeGasWellData(newWellNumber);
    setGasWells(prevWells => [...prevWells, newGasWell]);
  }, [gasWells]);
  
  // 移除天然气井
  const removeGasWell = useCallback((wellId) => {
    setGasWells(prevWells => prevWells.filter(well => well.id !== wellId));
  }, []);
  ...
  const updateCalculatedValues = useCallback(() => {
    // 处理天然气井数据计算
    setGasWells(prevWells => {
      let hasChanges = false;
      const updatedWells = prevWells.map(well => {
        if (!well.data) return well;
        
        let updatedWell = { ...well, data: { ...well.data } };
        let wellChanged = false;
        ...
        for (let monthIndex = 0; monthIndex < 12; monthIndex++) {
          ...
          // 获取当月的无阻流量
          const openFlowRateData = well.data.openFlowRate?.find(m => m.month === month);
          const openFlowRateValue = openFlowRateData?.value ? parseFloat(openFlowRateData.value) : 0;
          
          // 获取当月的作业时数
          const operatingHoursData = well.data.operatingHours?.find(m => m.month === month);
          const operatingHoursValue = operatingHoursData?.value ? parseFloat(operatingHoursData.value) : 0;

          // 获取当月的CH4体积浓度
          const ch4ConcentrationData = well.data.ch4Concentration?.find(m => m.month === month);
          const ch4ConcentrationValue = ch4ConcentrationData?.value ? parseFloat(ch4ConcentrationData.value) : 100;
          
          // 计算排放量：无阻流量 * 作业时数 * (CH4体积浓度/100) * 7.17 * 21
          const emissionValue = openFlowRateValue * operatingHoursValue * (ch4ConcentrationValue / 100) * 7.17 * 21;
          
          // 更新排放量数据
          const currentEmissionData = updatedWell.data.emission || [];
      ...
      return hasChanges ? updatedWells : prevWells;
    });
  }, [setGasWells]);
  ...
  }, [updateCalculatedValues, gasWells]);
  
  // 处理数据变化（纵向布局）
  const handleDataChange = useCallback((id, indicatorKey, month, field, value, type = 'gas-well') => {
    // 不进行格式化，直接使用原始输入值
    const formattedValue = value || '';
    
    // 使用天然气井相关设置
    const setState = setGasWells;
    const indicators = GAS_WELL_INDICATORS;
      ...
      const updatedItems = prevItems.map(item => {
        if (item.id === id) {
          ...
          if (monthIndex !== -1) {
            ...
            // 如果月份数据不存在，创建它
            hasChanges = true;
            const indicatorDefinition = GAS_WELL_INDICATORS.find(ind => ind.key === indicatorKey);
  ...
  const totalEmission = useMemo(() => {
    ...
    gasWells.forEach(well => {
      if (well.data && well.data['emission']) {
        well.data['emission'].forEach(monthData => {
  ...
  const calculateMonthlyEmissionTotals = () => {
    ...
    for (let month = 0; month < 12; month++) {
      ...
      gasWells.forEach(well => {
        if (well.data && well.data['emission']) {
          const emissionData = well.data['emission'];
  ...
  const renderTotalEmissionTable = () => {
    ...
    return (
      <table style={{ borderCollapse: 'collapse', width: '100%', marginTop: '24px', backgroundColor: '#f9f9f9' }}>
        ...
        <tbody>
          <tr>
            <td style={{ border: '1px solid #d9d9d9', padding: '8px', fontWeight: 'bold' }}>天然气井排放总量</td>
  ...
  return (
    <div style={{ padding: '16px' }}>
      <h2 style={{ marginBottom: '24px', fontSize: '20px', color: '#333' }}>油气勘探业务工艺放空CH4 排放</h2>
      ...
      <div style={{ marginTop: '20px',marginBottom: '40px' }}>
        <div style={{ marginBottom: '20px' }}>
          <div style={{ backgroundColor: '#f5f5f5', padding: '16px', borderRadius: '4px', lineHeight: '1.2' }}>
            ...
            <p>根据国内目前油气勘探工艺，油气勘探环节CH4 等易燃气体直接放空的情形很少。可能的情形是天然气试井时的无阻放空过程，通常都会加以回收或通过火炬燃烧处理。如果直接放空，则需计算CH4 排放量</p>
            <h3 style={{ marginBottom: '12px' }}>计算说明</h3>
            <p>天然气井的排放计算方法：排放量（tCO₂） = 无阻流量（m³/h） × 作业时数（h） × (CH4体积浓度/100) × 7.17 × 21</p>
            <p>- 无阻流量单位为 m³/h，保留两位小数</p>
            <p>- 作业时数单位为 h，保留两位小数</p>
            <p>- CH4体积浓度单位为 %，保留两位小数</p>
            <p>- 排放量单位为 tCO₂，保留两位小数</p>
            <h4 style={{ marginTop: '20px', marginBottom: '10px', fontSize: '16px' }}>系数说明：</h4>
            <p>- 7.17：CH₄密度转换系数，用于将CH₄体积浓度（%）转换为吨CH₄/10⁴Nm³</p>
            <p>- 21：CH₄的全球变暖潜势(GWP)值（100年时间尺度），用于将CH₄排放量转换为CO₂当量</p>
          </div>
        </div>
        ...
        <div style={{ marginTop: '24px', backgroundColor: '#fff', padding: '20px', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)', marginBottom: '24px' }}>
          <h3 style={{ marginBottom: '20px', color: '#1890ff', fontWeight: 'bold', fontSize: '18px' }}>添加实施无阻放空的天然气井排放记录</h3>

          <div style={{ display: 'flex', justifyContent: 'flex-start', gap: '16px', alignItems: 'end' }}>
            <button
              onClick={addNewGasWell}
              style={{
                padding: '8px 16px',
        ...
        <div>
          {gasWells.map((well, index) => (
            <div key={well.id} style={{ marginBottom: '32px', border: '1px solid #e8e8e8', padding: '16px', borderRadius: '4px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px' }}>
                <div style={{ display: 'flex', alignItems: 'center' }}>
                  <h3 style={{ margin: 0, fontSize: '16px', marginRight: '16px' }}>天然气井 {index + 1}</h3>
                </div>
                {(
                  <button
                    onClick={() => removeGasWell(well.id)}
                    style={{
                      padding: '4px 12px',
              ...
              {renderVerticalLayoutTable(well)}
            </div>
          ))}
        </div>
...
export default GasExplorationCH4Emission;
